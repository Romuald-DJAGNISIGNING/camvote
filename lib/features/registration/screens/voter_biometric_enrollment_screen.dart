...
class _VoterBiometricEnrollmentScreenState
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(t.biometricEnrollmentTitle)),
      body: BrandBackdrop(
              ...
              CamStagger(
                children: [
                  ...
                  _EnrollmentStepCard(
                    ...
                    buttonLabel:
                        enrollment.biometricEnrolled ? t.recheck : t.enrollNow,
                  ...
                  _EnrollmentStepCard(
                    ...
                    buttonLabel:
                        enrollment.livenessVerified ? t.recheck : t.runLiveness,
  ...
  Future<void> _handleBiometric(BuildContext context) async {
    ...
    final ok = await gate.requireBiometric(
      reason: t.biometricEnrollReason,
    );
    if (!mounted) return;
    if (!ok) {
    ...
    await ref.read(registrationEnrollmentProvider.notifier).markBiometricEnrolled();
    if (!mounted) return;
  ...
  Future<void> _handleLiveness(BuildContext context) async {
    ...
    await ref.read(registrationEnrollmentProvider.notifier).markLivenessVerified();
    if (!mounted) return;
...
class _EnrollmentStepCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final badgeColor =
        complete ? cs.primaryContainer : cs.surfaceContainerHighest;
    final t = AppLocalizations.of(context);
